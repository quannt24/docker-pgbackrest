...
```bash
docker run --rm -it pgbackrest:2.57.0 bash
...
```bash
make build_version TAG=2.57.0
```

```bash
make build_version_alpine TAG=2.57.0
```

or

```bash
docker build -f Dockerfile --build-arg BACKREST_VERSION=2.57.0 --build-arg BACKREST_COMPLETION_VERSION=v0.10 -t pgbackrest:2.57.0 .
```

```bash
docker build -f Dockerfile.alpine --build-arg BACKREST_VERSION=2.57.0 --build-arg BACKREST_COMPLETION_VERSION=v0.10 -t pgbackrest:2.57.0-alpine .
```
